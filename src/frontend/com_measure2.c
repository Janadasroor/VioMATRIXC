...
measure_function_type(char *operation)
{
    ...
    else if (strcasecmp(mFunction, "DERIVATIVE") == 0)
        mFunctionType = AT_DERIV;
...
err_ret7:
    ...
    case AT_DERIV:
    {
        /* Compute numerical derivative using finite differences.
           Supports: DERIV V(out) AT=<val>  and  DERIV V(out) WHEN V(in)=<val> RISE=n */
        MEASUREPTR measDeriv;
        struct dvec *dv = NULL, *dScale = NULL;
        double deriv_val = 0.0;
        int deriv_ok = 0;

        measDeriv = TMALLOC(struct measure, 1);
        measDeriv->m_analysis = mAnalysis;
        measDeriv->m_vec = NULL;
        measDeriv->m_vec2 = NULL;
        measDeriv->m_vectype = 'm';
        measDeriv->m_vectype2 = 'm';
        measDeriv->m_rise = 1;
        measDeriv->m_fall = 0;
        measDeriv->m_cross = 0;
        measDeriv->m_val = 0.0;
        measDeriv->m_td = 0.0;
        measDeriv->m_from = MEASURE_DEFAULT;
        measDeriv->m_to = MEASURE_DEFAULT;
        measDeriv->m_at = -1.0;
        measDeriv->m_measured = 0.0;
        measDeriv->m_measured_at = 0.0;

        if (measure_parse_trigtarg(measDeriv, words, NULL, "trig", errbuf) ==
                MEASUREMENT_FAILURE) {
            measure_errMessage(mName, mFunction, "TRIG", errbuf, autocheck);
            goto err_ret8;
        }

        /* Fix up vector name (e.g. vm(out) -> v(out), set m_vectype) */
        correct_vec(measDeriv);

        /* Get the vector to differentiate */
        dv = vec_get(measDeriv->m_vec);
        if (!dv) {
            sprintf(errbuf, "vector '%s' not found\n", measDeriv->m_vec);
            measure_errMessage(mName, mFunction, "", errbuf, autocheck);
            goto err_ret8;
        }

        /* Get the scale vector (time/frequency) */
        if (cieq(measDeriv->m_analysis, "ac") || cieq(measDeriv->m_analysis, "sp"))
            dScale = vec_get("frequency");
        else if (cieq(measDeriv->m_analysis, "dc"))
            dScale = vec_get(measDeriv->m_vec); /* DC sweep uses source name */
        else
            dScale = vec_get("time");

        if (!dScale) {
            sprintf(errbuf, "no scale vector available\n");
            measure_errMessage(mName, mFunction, "", errbuf, autocheck);
            goto err_ret8;
        }

        if (measDeriv->m_at > 0) {
            /* DERIV V(out) AT=<val> — compute derivative at a specific point
               using central difference with neighboring points */
            double at_time = measDeriv->m_at;
            int idx = -1;
            /* Find closest index bracketing at_time */
            for (int i = 0; i < dScale->v_length - 1; i++) {
                if (dScale->v_realdata[i] <= at_time && dScale->v_realdata[i+1] >= at_time) {
                    idx = i;
                    break;
                }
            }
            ...
            if (idx >= dScale->v_length - 1) {
                idx = dScale->v_length - 2;
            }
            /* Central difference: f'(x) ≈ (f(x+h) - f(x-h)) / (2h) */
            double h = dScale->v_realdata[idx+1] - dScale->v_realdata[idx-1];
            if (h != 0.0 && dv->v_flags & VF_REAL) {
                deriv_val = (dv->v_realdata[idx+1] - dv->v_realdata[idx-1]) / h;
                deriv_ok = 1;
            } else if (h != 0.0) {
                deriv_val = (dv->v_compdata[idx+1].cx_real - dv->v_compdata[idx-1].cx_real) / h;
                deriv_ok = 1;
            }
            measDeriv->m_measured = deriv_val;
            measDeriv->m_measured_at = at_time;
        } else {
            /* DERIV V(out) WHEN V(in)=<val> RISE=n — compute derivative when
               triggering condition is met. */
            double trigger_time = measDeriv->m_measured_at;
            int idx = -1;
            for (int i = 0; i < dScale->v_length - 1; i++) {
                if (dScale->v_realdata[i] <= trigger_time && dScale->v_realdata[i+1] >= trigger_time) {
                    idx = i;
                    break;
                }
            }
            if (idx < 1) idx = 1;
            if (idx >= dScale->v_length - 1) idx = dScale->v_length - 2;
            ...
            measDeriv->m_measured = deriv_val;
            measDeriv->m_measured_at = trigger_time;
        }

        if (!deriv_ok) {
            sprintf(errbuf, "could not compute derivative — insufficient data points\n");
            measure_errMessage(mName, mFunction, "", errbuf, autocheck);
            goto err_ret8;
        }
        ...
        if (out_line)
            sprintf(out_line, "%-20s=  %.*e at=  %.*e\n",
                mName, precision, measDeriv->m_measured, precision, measDeriv->m_measured_at);
        else
            fprintf(mout, "%-20s=  %.*e at=  %.*e\n",
                mName, precision, measDeriv->m_measured, precision, measDeriv->m_measured_at);

        *result = measDeriv->m_measured;
        ret_val = MEASUREMENT_OK;

err_ret8:
        tfree(mAnalysis);
        tfree(mName);
        if (measDeriv) tfree(measDeriv->m_vec);
        tfree(measDeriv);
        tfree(mFunction);
        return ret_val;
    }
