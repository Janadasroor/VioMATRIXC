...
static SendConvInfo* convinfofcn = NULL;
static convergence_info last_conv_info = {0};
static measure_result_all last_measure_results = {0, NULL};
...
void (*ng_convergence_callback)(pconvergence_info) = NULL;

/* Initialise ngspice with extended callbacks including convergence diagnostics.
   Same as ngSpice_Init but adds a convergence info callback. */
IMPEXP
int
ngSpice_Init_Extended(SendChar* printfcn, SendStat* statusfcn,
             ControlledExit* ngspiceexit, SendData* sdata,
             SendInitData* sinitdata, BGThreadRunning* bgtrun,
             SendConvInfo* convinfo, void* userData)
{
    int ret = ngSpice_Init(printfcn, statusfcn, ngspiceexit,
                           sdata, sinitdata, bgtrun, userData);
    convinfofcn = convinfo;
    /* If caller provided a convergence callback, store it for analysis code */
    ng_convergence_callback = (convinfo != NULL)
        ? (void(*)(pconvergence_info))convinfo
        : NULL;
    /* convinfo may be NULL — caller may not want convergence callbacks */
    return ret;
}
...
/* Return structured info about the current plot including all vectors.
   Can be called after loading a netlist (before run) to let the caller
   populate signal lists even before simulation completes.
   The caller must NOT free the returned pointer or its contents.

   Skips the internal "const" plot and returns the most recent circuit
   plot (tran, ac, dc, op, sp, etc.). */
IMPEXP
pvecinfoall
ng_get_plot_info(void)
{
    struct plot *pl, *best = NULL;
    struct dvec *d;
    ...
    /* Find the last non-constant plot in the list */
    pl = plot_list;
    while (pl) {
        if (!pl->pl_typename || !cieq(pl->pl_typename, "constants"))
            best = pl;
        pl = pl->pl_next;
    }
    ...
    if (!best) {
        if (ft_ngdebug)
            fprintf(stdout, "ng_get_plot_info: no circuit plot available\n");
        return NULL;
    }
    ...
    if (pvca) {
        if (pvca->vecs) {
            for (i = 0; i < pvca->veccount; i++) {
                /* strings point to dvec internals, don't free them */
                tfree(pvca->vecs[i]);
            }
            tfree(pvca->vecs);
        }
    ...
    pvca->name = copy(pl->pl_typename);
    pvca->title = pl->pl_title ? copy(pl->pl_title) : copy("");
    pvca->date = pl->pl_date ? copy(pl->pl_date) : copy("");
    pvca->type = copy(pl->pl_name);
    pvca->veccount = len;

    if (len > 0) {
        pvca->vecs = TMALLOC(pvecinfo, len);
        for (d = pl->pl_dvecs, i = 0; d; d = d->v_next, i++) {
            pvca->vecs[i] = TMALLOC(vecinfo, 1);
            pvca->vecs[i]->number = i;
            pvca->vecs[i]->vecname = d->v_name;
            pvca->vecs[i]->is_real = (d->v_flags & VF_REAL) ? TRUE : FALSE;
            pvca->vecs[i]->pdvec = (void*)d;
            pvca->vecs[i]->pdvecscale = (void*)d->v_scale;
        }
    } else {
        pvca->vecs = NULL;
    }
...
IMPEXP
pmeasure_result_all
ng_get_measure_results(void)
{
    return (last_measure_results.count > 0) ? &last_measure_results : NULL;
}
