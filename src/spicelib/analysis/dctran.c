...
*/
        ...
        if (converged != 0) {
            ng_report_convergence("tran-op", converged, "op-failed",
                                  0, 0, ckt->CKTstat->STATnumIter,
                                  ckt->CKTdcTrcvMaxIter, 0,
                                  "Transient operating point failed");
...
/* Call all hybrids */
        ...
        /* Report convergence diagnostics only on significant events:
           failure, non-convergence, first step, or last step */
        {
            int is_first = (ckt->CKTstat->STATtimePts == 0);
            int is_last = (ckt->CKTtime + ckt->CKTdelta >= ckt->CKTfinalTime);
            if (converged != 0 || is_first || is_last) {
                ng_report_convergence("tran", converged, "newton-raphson",
                                      ckt->CKTtime, ckt->CKTdelta,
