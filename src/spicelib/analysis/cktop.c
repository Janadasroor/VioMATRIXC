       ...
       int iterlim)
{
    ...
    if (!ckt->CKTnoOpIter) {
        ...
        if (converged == 0) {
            ng_report_convergence("op", 0, "direct", 0, 0,
                                  ckt->CKTstat->STATnumIter, iterlim, 0, NULL);
    ...
    ng_report_convergence("op", converged, "all-methods-exhausted",
                          0, 0, ckt->CKTstat->STATnumIter, iterlim, 0,
                          "Operating point failed: gmin stepping, source stepping, and optran all failed");
