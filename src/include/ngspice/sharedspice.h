...
/* Convergence diagnostics info */
typedef struct convergence_info {
    char    *analysis_name;    /* "op", "tran", "ac", "dc" */
    NG_BOOL  converged;        /* TRUE if simulation converged */
    int      iterations;       /* Newton-Raphson iterations for last step */
    int      max_iterations;   /* iteration limit that was hit */
    char    *method_tried;     /* "direct", "gmin", "source", "tran-op" */
    NG_BOOL  method_succeeded; /* whether the attempted method worked */
    double   ckt_time;         /* simulation time at failure (for tran) */
    double   ckt_delta;        /* timestep at failure (for tran) */
    double   residual_max;     /* maximum residual in the system */
    int      error_code;       /* ngspice error code (0=success) */
    char    *detail_message;   /* human-readable diagnostic string */
} convergence_info, *pconvergence_info;

/* Measurement function types matching com_measure2.c enum */
typedef enum {
    MEAS_UNKNOWN = 0,
    MEAS_AVG,
    MEAS_RMS,
...
typedef struct measure_result {
    char      *name;       /* result name from .measure */
    double     value;      /* numeric result value */
    meas_type_t type;      /* measurement type (AVG, RMS, MAX, etc.) */
    char      *unit;       /* unit string (e.g. "V", "A", "s", "Hz", "") */
    char      *from_val;   /* FROM= value if applicable */
    char      *to_val;     /* TO= value if applicable */
    char      *at_val;     /* AT= value if applicable */
    char      *trig_val;   /* TRIG= value if applicable */
    char      *targ_val;   /* TARG= value if applicable */
} measure_result, *pmeasure_result;

/* Array of measurement results */
typedef struct measure_result_all {
    int              count;       /* number of results */
    pmeasure_result *results;     /* array of result pointers */
} measure_result_all, *pmeasure_result_all;

/* callback function for convergence diagnostics */
typedef int (SendConvInfo)(pconvergence_info, int, void*);
/*
   convergence_info*  pointer to struct with convergence diagnostics
   int                identification number of calling ngspice shared lib
   void*              return pointer received from caller
*/

/* ngspice initialization with extended callbacks
   Same as ngSpice_Init but adds convergence diagnostics callback.
   convinfo: pointer to callback function for convergence diagnostics
   userData: pointer to user-defined data */
IMPEXP
int ngSpice_Init_Extended(SendChar* printfcn, SendStat* statfcn,
                  ControlledExit* ngexit, SendData* sdata,
                  SendInitData* sinitdata, BGThreadRunning* bgtrun,
                  SendConvInfo* convinfo, void* userData);

/* Return structured info about the current plot including all vectors.
   This can be called after loading a netlist (before run) and after
   simulation completes. Returns NULL if no plot exists. */
IMPEXP
pvecinfoall ng_get_plot_info(void);

/* Return all accumulated .measure results as structured data.
   Returns NULL if no measurements have been executed.
   Caller must NOT free the returned pointer or its contents. */
IMPEXP
pmeasure_result_all ng_get_measure_results(void);

/* Internal function for analysis code to report convergence status.
   Called from CKTop, dctran, etc. Not intended for external callers. */
void ng_report_convergence(const char *analysis, int converged,
                           const char *method, double ckt_time,
                           double ckt_delta, int iterations,
                           int max_iter, double residual,
                           const char *detail);
