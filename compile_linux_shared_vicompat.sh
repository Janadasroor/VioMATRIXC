...
# (depending on the Linux distribution, and on the calling programs search path).
  ../configure --with-ngshared --enable-cider --enable-vicompat --prefix=/usr CFLAGS="-g -m64 -O0 -Wall" LDFLAGS="-m64 -g"
else
   cd releasesh
...
# The --prefix (and perhaps --libdir) may be used to determine a different install location
# (depending on the Linux distribution, and on the calling programs search path).
  ../configure --with-ngshared --enable-cider --enable-vicompat --prefix=/usr CFLAGS="-m64 -O2" LDFLAGS="-m64 -s"
fi
